...
import { CITIES } from "@/components/three/EarthScene";

const EarthScene = dynamic(() => import("@/components/three/EarthScene"), { ssr: false });

type Label = { name: string; x: number; y: number; visible: boolean; home?: boolean };
...
export default function GlobalMission() {
  ...
  return (
    <section className="relative py-12">
      ...
      <div className="relative mx-auto w-full overflow-hidden md:px-5">
        <div className="relative h-[560px] w-full overflow-hidden rounded-none bg-[radial-gradient(80%_70%_at_50%_45%,#0b1638_0%,#03040a_70%)] md:h-[720px] md:rounded-3xl md:shadow-border">
          ...
          <div className="pointer-events-none absolute inset-0">
            {labels.map((l) => (
              <span
                key={l.name}
                className={`absolute -translate-x-1/2 -translate-y-[160%] whitespace-nowrap rounded-full px-2 py-0.5 font-mono text-[10px] transition-opacity duration-300 ${
                  l.home ? "bg-emerald-400/15 text-emerald-300 ring-1 ring-emerald-400/40" : "bg-black/40 text-white/75"
                }`}
          ...
          >
            ...
            <div className="flex items-start gap-2 py-1">
              ...
              <p className="text-white/70">5 satellites · 28 flights tracked</p>
            </div>
          </motion.div>
          ...
          <p className="pointer-events-none absolute bottom-3 left-1/2 z-10 flex -translate-x-1/2 items-center gap-2 rounded-full bg-black/40 px-3 py-1 font-mono text-[10px] uppercase tracking-widest text-white/60 backdrop-blur">
            <Hand className="size-3.5" /> Drag to explore
          </p>
        </div>
      </div>
    </section>
